...
export default async function addUser(
  ...
  if (!fullname || !password || !email) {
    throw new AppError(
      "fullname, password and email fields cannot be empty",
      403,
    );
  }

  const hashedPassword = await authService.createHashedPassword(password);
  const newUser = user(fullname, hashedPassword, email, role, createdAt);

  return userRepository.findByProperty({ email }).then((userWithEmail) => {

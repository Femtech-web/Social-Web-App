...
const handleSubmit = async (e, selectedFiles, cat, editId, fullname, inputText, 
  dispatch, setInputText, setSelectedFiles, navigate) => {
  e.preventDefault();
  dispatch(fetchStart())
...
const handleDone = (dispatch, selectedCat, setCategoryActive, populateCategory) => {
...
const handleSearch = (e, categories, setSearchedCat) => {
  ...
  const searchedWords = categories.filter((item) => item.includes(searchedTerm));

  setSearchedCat(searchedWords);
};


export { initialText, handleChange, handleFileUpload, 
handleSubmit, handleClick, handleDone, handleSearch};